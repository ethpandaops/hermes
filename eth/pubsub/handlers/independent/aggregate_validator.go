...
func (v *AggregateAndProofValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
	...
	if err := aggregateAndProof.UnmarshalSSZ(decompressed); err != nil {
		return nil, errors.Wrap(err, "failed to decode aggregate and proof")
	}

	if aggregateAndProof.Message == nil || aggregateAndProof.Message.Aggregate == nil {
		return nil, errors.New("nil aggregate or proof")
	}
	...
	if common.Slot(aggregate.Data.Slot) > currentSlot {
		return nil, fmt.Errorf("attestation slot %d is from the future (current slot: %d)", aggregate.Data.Slot, currentSlot)
	}
	...
	if !validatorInfo.Active {
		return nil, fmt.Errorf("aggregator %d is not active in epoch %d", aggregatorIndex, currentEpoch)
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to compute selection proof domain")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to compute selection root")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to compute aggregate domain")
	}

	aggregateRoot, err := common.ComputeSigningRoot(msg, aggregateDomain)
	if err != nil {
		return nil, errors.Wrap(err, "failed to compute aggregate signing root")
	}
	...
	if err := v.validator.signatureVerifier.VerifySignature(
		...
		return nil, errors.Wrap(err, "invalid aggregate signature")
	}
