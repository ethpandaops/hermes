...
func (v *SyncCommitteeMessageValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
	...
	if err := msg.UnmarshalSSZ(decompressed); err != nil {
		return nil, errors.Wrap(err, "failed to decode sync committee message")
	}

	// Get current state
	state := v.validator.stateSync.GetCurrentState()
	if state == nil {
		return nil, errors.New("current state not available")
	}

	// Validate slot
	currentSlot := state.Slot
	if common.Slot(msg.Slot) > currentSlot {
		return nil, fmt.Errorf("sync committee message slot %d is from the future (current slot: %d)", msg.Slot, currentSlot)
	}
	...
	if period == currentPeriod {
		...
		return nil, fmt.Errorf("sync committee message is for period %d, but current period is %d", period, currentPeriod)
	}

	if syncCommittee == nil {
		return nil, errors.New("sync committee not available for validation period")
	}

	// Verify validator is in sync committee
	if msg.ValidatorIndex >= phase0.ValidatorIndex(len(syncCommittee.ValidatorIndices)) {
		return nil, fmt.Errorf("validator index %d not in sync committee", msg.ValidatorIndex)
	}

	// Get validator's public key
	validatorInfo, exists := state.Validators[common.ValidatorIndex(msg.ValidatorIndex)]
	if !exists {
		return nil, fmt.Errorf("validator %d not found in validator set", msg.ValidatorIndex)
	}
...
func (v *SyncCommitteeContributionValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
	...
	if err := contributionAndProof.UnmarshalSSZ(decompressed); err != nil {
		return nil, errors.Wrap(err, "failed to decode contribution and proof")
	}

	if contributionAndProof.Message == nil || contributionAndProof.Message.Contribution == nil {
		return nil, errors.New("nil contribution or proof")
	}
	...
	if common.Slot(contribution.Slot) > currentSlot {
		return nil, fmt.Errorf("contribution slot %d is from the future (current slot: %d)", contribution.Slot, currentSlot)
	}
	...
	if !exists {
		return nil, fmt.Errorf("aggregator %d not found in validator set", aggregatorIndex)
	}
	...
	if err := v.validator.signatureVerifier.VerifySignature(
		...
		return nil, errors.Wrap(err, "invalid selection proof")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to serialize contribution and proof")
	}
