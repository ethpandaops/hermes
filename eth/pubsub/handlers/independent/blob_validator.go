...
func (v *BlobSidecarValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
	...
	if err := sidecar.UnmarshalSSZ(decompressed); err != nil {
		return nil, errors.Wrap(err, "failed to decode blob sidecar")
	}

	// Verify blob index is within bounds
	if sidecar.Index >= common.MAX_BLOBS_PER_BLOCK {
		return nil, fmt.Errorf("blob index %d exceeds max %d", sidecar.Index, common.MAX_BLOBS_PER_BLOCK)
	}
	...
	if err := v.verifyInclusionProof(sidecar); err != nil {
		return nil, errors.Wrap(err, "inclusion proof verification failed")
	}

	// Verify proposer signature on block header
	if err := v.verifyProposerSignature(sidecar); err != nil {
		return nil, errors.Wrap(err, "proposer signature verification failed")
	}
