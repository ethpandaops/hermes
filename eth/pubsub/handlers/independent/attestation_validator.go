...
func (v *StandardAttestationValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
	...
	// Get attester index from aggregation bits
	attesterIndex := v.getAttesterIndex(committee, attestation.AggregationBits)
	if attesterIndex == nil {
		return nil, errors.New("no attester index found in aggregation bits")
	}
