...
// Handle validates and decodes a beacon block
func (v *BeaconBlockValidator) Handle(ctx context.Context, data []byte, topic string) (interface{}, error) {
				...
				if err = bellatrixBlock.UnmarshalSSZ(decompressed); err == nil {
					...
					if err = altairBlock.UnmarshalSSZ(decompressed); err == nil {
						...
						if err = phase0Block.UnmarshalSSZ(decompressed); err == nil {
							...
							return nil, errors.Wrap(err, "failed to decode beacon block")
						}
					}
				}
			}
	...
	if common.Slot(slot) > currentSlot {
		return nil, fmt.Errorf("block slot %d is from the future (current slot: %d)", slot, currentSlot)
	}

	// 2. Check slot is recent (not too old)
	if currentSlot > common.Slot(slot) && currentSlot-common.Slot(slot) > 64 { // 64 slots = ~13 minutes
		return nil, fmt.Errorf("block slot %d is too old (current slot: %d)", slot, currentSlot)
	}
	...
	if !exists {
		return nil, fmt.Errorf("proposer %d not found in validator set", proposerIndex)
	}
