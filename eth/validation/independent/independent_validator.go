...
func NewIndependentValidator(config *IndependentConfig) (*IndependentValidator, error) {
	...
	stateProvider := NewHTTPStateProvider(config.BeaconNodeEndpoint)
	...
	stateSync := NewBeaconStateSyncer(logger, config.BeaconNodeEndpoint, config.StateUpdateInterval)
